...
_PYGAME_KEY_MAP: Dict[int, int] = {
    ...
    pygame.K_SPACE: int(Key.PLAY),
    pygame.K_p: int(Key.PAUSE),
    pygame.K_PLUS: int(Key.VOLUME_UP),
    pygame.K_KP_PLUS: int(Key.VOLUME_UP),
    pygame.K_MINUS: int(Key.VOLUME_DOWN),
    pygame.K_KP_MINUS: int(Key.VOLUME_DOWN),
    pygame.K_PAGEUP: int(Key.PAGE_UP),
    pygame.K_PAGEDOWN: int(Key.PAGE_DOWN),
    pygame.K_m: int(Key.MUTE),
    pygame.K_s: int(Key.STOP),
    pygame.K_0: int(Key.PRESET_0),
    pygame.K_1: int(Key.PRESET_1),
    pygame.K_2: int(Key.PRESET_2),
    pygame.K_3: int(Key.PRESET_3),
    pygame.K_4: int(Key.PRESET_4),
    pygame.K_5: int(Key.PRESET_5),
    pygame.K_6: int(Key.PRESET_6),
    pygame.K_7: int(Key.PRESET_7),
    pygame.K_8: int(Key.PRESET_8),
    pygame.K_9: int(Key.PRESET_9),
}
...
class Framework:
    ...
    def add_listener(
        ...
        Higher *priority* values are checked later (allowing earlier
        listeners to consume the event first).  Default priority 0 means
        the listener is prepended.
        ...
        if priority <= 0:
            ...
            self._global_listeners.append(handle)
        return handle
        ...
        for handle in self._global_listeners:
            ...
            if etype & mask:
                try:
                    ...
                    log.warn(f"global listener error: {exc}")
            ...
            for handle in self._unused_listeners:
                ...
                if etype & mask:
                    try:
                        ...
                        log.warn(f"unused listener error: {exc}")
            ...
            for anim in list(widget.animations):
                ...
                if anim[2] <= 0:
                    ...
                    try:
                        ...
                        log.warn(f"animation error: {exc}")

    # ==================================================================
    # Screen update
    # ==================================================================

    def re_draw(self, rect: Optional[Tuple[int, int, int, int]]) -> None:
        """
        Mark *rect* (or the whole screen if ``None``) for redrawing.
        """
        # In our implementation we always do a full redraw each frame,
        # so this is currently a no-op placeholder.  A future optimisation
        # could track dirty rectangles.
        pass
    ...
    def update_screen(self) -> None:
        ...
        # Background — clear to black if no bg tile/image
        if self._background is not None:
            # If background is a Surface or Tile, blit it
            try:
                self._background.blit(screen, 0, 0)
            except Exception:
        ...
        if self._transition is not None:
            ...
            try:
                ...
                log.warn(f"transition error: {exc}")
    ...
    def load_sound(self, name: str, file: str, channel: int = 0) -> None:
        ...
        try:
            ...
            log.warn(f"Failed to load sound '{name}': {exc}")
    ...
    def get_action_to_action_translation(self, action_name: str) -> Optional[str]:
        ...
        if table is not None:
            return table.get(action_name)
        return None

    getActionToActionTranslation = get_action_to_action_translation

    def set_action_to_action_translation(
        self, source: str, target: Optional[str]
    ) -> None:
        """
    ...
    def style_changed(self) -> None:
        ...
        Re-skins all windows and global widgets.
        """
        ...
        for pg_event in pygame.event.get():
            if pg_event.type == pygame.QUIT:
                ...
                if jive_key:
                    ...
                    self.push_event(
                        Event(int(EVENT_KEY_DOWN), code=jive_key, ticks=now)
                    )
                # Also generate CHAR_PRESS for printable characters
                if pg_event.unicode and ord(pg_event.unicode) >= 32:
                    self.push_event(
                        Event(
        ...
        for jive_key, down_ticks in list(self._keys_down.items()):
            if jive_key not in self._keys_held:
                if now - down_ticks >= _KEY_HOLD_TIME:
                    ...
                    self.push_event(
                        Event(int(EVENT_KEY_HOLD), code=jive_key, ticks=now)
                    )
    ...
    def event_loop(self) -> None:
        ...
        2. Process timer queue
        3. Tick animations
        4. Process event queue
        5. Update / draw the screen
        ...
        # Add the input-to-action converter as a low-priority global listener
        self.add_listener(
            int(EVENT_KEY_ALL)
            | int(EVENT_CHAR_PRESS)
            | int(EVENT_IR_ALL)
        ...
        while self._running:
            ...
            # 5. Guard: quit() may have been called from a timer/listener
    ...
    def _register_default_actions(self) -> None:
        ...
        default_key_maps = [
            ...
            (int(EVENT_KEY_PRESS), int(Key.LEFT), "title_left"),
            (int(EVENT_KEY_PRESS), int(Key.RIGHT), "title_right"),
            # Scroll via keys
            (int(EVENT_KEY_PRESS), int(Key.UP), "scroll_up"),
            (int(EVENT_KEY_PRESS), int(Key.DOWN), "scroll_down"),
        ]
