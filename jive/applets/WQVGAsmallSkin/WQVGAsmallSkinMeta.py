"""
jive.applets.WQVGAsmallSkin.WQVGAsmallSkinMeta --- Meta class for WQVGAsmallSkin.

Ported from ``share/jive/applets/WQVGAsmallSkin/WQVGAsmallSkinMeta.lua``
in the original jivelite project.

WQVGAsmallSkin is a small-print skin for 480x272 resolution displays.
...
__all__ = ["WQVGAsmallSkinMeta"]

log = logger("applet.WQVGAsmallSkin")


class WQVGAsmallSkinMeta(AppletMeta):
    """Meta-information for the WQVGAsmallSkin applet.

    Registers the 480x272 small-print skin with JiveMain so that it
    ...
    def register_applet(self) -> None:
        """Register the WQVGAsmallSkin with JiveMain."""
        jive_main = self._get_jive_main()
        if jive_main is None:
            log.warn("WQVGAsmallSkinMeta: jiveMain not available for skin registration")
            return

        jive_main.register_skin(
            str(self.string("WQVGA_SMALL_SKIN")),
            "WQVGAsmallSkin",
            "skin",
        )
