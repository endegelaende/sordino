"""
jive.applets.HDGridSkin.HDGridSkinApplet --- HDGridSkin applet.

Ported from ``share/jive/applets/HDGridSkin/HDGridSkinApplet.lua``
(~3210 LOC) in the original jivelite project.

This applet implements an HD grid skin for 1080p resolution displays.
It is a standalone skin that inherits directly from Applet and builds
the complete style table from scratch, using a grid layout for the
home menu and icon lists.

Key implementation notes
------------------------

* Standalone skin — does NOT inherit from QVGAbaseSkin or JogglerSkin.
* Default screen size is 1920x1080 (1080p).
* Single resolution variant via ``skin_1080p`` method.
* Grid layout for home menu and icon lists with large thumbnails.
...
# ---------------------------------------------------------------------------
_IMGPATH = "applets/HDGridSkin/images/"
_FONTPATH = "fonts/"
_FONT_NAME = "FreeSans"
_BOLD_PREFIX = "Bold"
...
def _load_image(file: str) -> Style:
    """Return a lazy reference to an image in the HDGridSkin images dir."""
...
# Default cover art size
_coverSize = 900


class HDGridSkinApplet(Applet):
    """Skin applet for HD 1080p displays with grid layout.

    Standalone skin that builds the complete style table from scratch.
    Uses a grid layout for home menu and icon lists with large thumbnails.
    """
    ...
    def param(self) -> Style:
        """Return skin parameter constants for the HD grid skin."""
    ...
    def _skin_common(
        ...
        """Populate *s* with the complete HDGridSkin style table.
        ...
        try:
            ...
            log.warning(
                "HDGridSkin: failed to set initial video mode (%sx%s): %s",
                target_width,
                target_height,
                exc,
            )
        ...
        try:
            ...
            log.warning(
                "HDGridSkin: failed to set fullscreen video mode (%sx%s): %s",
                screen_width,
                screen_height,
                exc,
            )
        ...
        try:
            ...
            log.debug("HDGridSkin: failed to set most_recent_input_type: %s", exc)
        ...
        # ---------------------------------------------------------------
        # Title-bar buttons (Lua L2269-2366)
        # ---------------------------------------------------------------
        _button: Style = {
            "bgImg": titlebarButtonBox,
        ...
        # Invisible button (Lua L2335-2339)
        s["button_none"] = _uses(
            _button,
            {
                "bgImg": False,
        ...
        # Volume min/max buttons (Lua L2357-2365)
        s["button_volume_min"] = {
    ...
    def skin_1080p(
        ...
        """Skin variant for 1080p (1920x1080) grid displays."""
        global _coverSize
        _coverSize = 900
