"""
jive.applets.WQVGAlargeSkin.WQVGAlargeSkinApplet --- WQVGAlargeSkin applet.

Ported from ``share/jive/applets/WQVGAlargeSkin/WQVGAlargeSkinApplet.lua``
(~2655 LOC) in the original jivelite project.

This applet implements a large-print skin for 480x272 resolution displays.
...
* Remote-oriented UI with 3-line item lists (larger fonts than WQVGAsmall).
...
# ---------------------------------------------------------------------------
# NOTE: The Lua original for WQVGAlargeSkin reuses WQVGAsmallSkin's image
# directory (``local imgpath = "applets/WQVGAsmallSkin/images/"``).
...
def _load_image(file: str) -> Style:
    """Return a lazy reference to an image in the WQVGAlargeSkin images dir."""
...
# ============================================================================
# WQVGAlargeSkinApplet
# ============================================================================


class WQVGAlargeSkinApplet(Applet):
    """Skin applet for 480x272 large-print displays.

    Standalone skin that builds the complete style table from scratch.
    Uses larger fonts and fewer items per screen than WQVGAsmallSkin.
    """
    ...
    def param(self) -> Style:
        """Return skin parameter constants for the 480x272 large-print skin."""
        return {
            "THUMB_SIZE": 64,
            "THUMB_SIZE_MENU": 64,
    ...
    def skin(
        ...
        """Populate *s* with the complete WQVGAlargeSkin style table.
        ...
        try:
            ...
            log.warning(
                "WQVGAlargeSkin: failed to set video mode (%sx%s): %s",
                screen_width,
                screen_height,
                exc,
            )
        ...
        try:
            ...
            log.debug("WQVGAlargeSkin: failed to set most_recent_input_type: %s", exc)

        imgpath = _IMGPATH

        # skin suffix — WQVGAlarge uses 'remote' skin variant
        thisSkin = "remote"
        ...
        # WQVGAlarge uses larger fonts and taller items than WQVGAsmall
        TITLE_HEIGHT = 47
        TITLE_FONT_SIZE = 20
        ALBUMMENU_FONT_SIZE = 24
        ALBUMMENU_SMALL_FONT_SIZE = 18
        ...
        # text_list — basic list with title (WQVGAlarge uses 3-line item height)
        s["text_list"] = _uses(
            s["window"],
            {
                "menu": _uses(
