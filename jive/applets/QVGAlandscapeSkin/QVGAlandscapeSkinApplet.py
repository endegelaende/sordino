"""
jive.applets.QVGAlandscapeSkin.QVGAlandscapeSkinApplet --- QVGAlandscapeSkin applet.

Ported from ``share/jive/applets/QVGAlandscapeSkin/QVGAlandscapeSkinApplet.lua``
(~367 LOC) in the original jivelite project.

This applet implements the skin for a 320x240 landscape display, such as
the Squeezebox Controller.  It inherits from QVGAbaseSkinApplet and
overrides a small number of styles for the landscape form factor.

Key implementation notes
------------------------

* Inherits the complete QVGA base skin via
  ``QVGAbaseSkinApplet.skin()``, then applies landscape-specific
  overrides (scrollbar, progress bar, NowPlaying layout, etc.).

* Default screen size is 320x240.

* NowPlaying styles are adapted for the landscape display:
...
__all__ = ["QVGAlandscapeSkinApplet"]
...
# Image path prefixes
# ---------------------------------------------------------------------------
_IMGPATH = "applets/QVGAlandscapeSkin/images/"
...
def _load_image(file: str) -> Style:
    """Return a lazy reference to an image in the QVGAlandscapeSkin images dir."""
...
class QVGAlandscapeSkinApplet(QVGAbaseSkinApplet):
    """Skin applet for 320x240 landscape displays (Squeezebox Controller, etc.).

    Inherits the complete QVGA base skin and overrides styles specific
    to the landscape form factor.
    """
    ...
    def param(self) -> Style:
        """Return skin parameter constants for the 320x240 landscape skin."""
        return {
            "THUMB_SIZE": 41,
            "THUMB_SIZE_MENU": 40,
    ...
    def skin(
        ...
        """Populate *s* with 320x240-landscape-specific style overrides.
        ...
        use_default_size:
            ``True`` to use the default 320x240 screen size instead of
        ...
        """
        # Screen dimensions — default to 320x240 for the landscape skin
        screen_width = 320
        screen_height = 240
        ...
        try:
            ...
            if _fw is not None:
                _fw.set_video_mode(320, 240, 0, False)
        ...
        if use_default_size or screen_width < 320 or screen_height < 240:
            screen_width = 320
            screen_height = 240
        ...
        # Sliders — hidden on landscape skin
